...
async function handleInstitutionRegistration(user: ExtendedUser) {
    try {
        ...
        console.log(`Created organization ${organization.id} for institution user ${user.id}`);
    } catch (error) {
        console.error(`Error creating organization for institution user ${user.id}:`, error);
    }
}
...
async function handleOrganizationRegistration(user: ExtendedUser) {
    try {
        ...
        console.log(`Created organization ${organization.id} for organization user ${user.id}`);
    } catch (error) {
        console.error(`Error creating organization for organization user ${user.id}:`, error);
    }
}
    ...
    try {
        ...
        if (user.instituteId) {
            ...
            if (faculty) {
                ...
                console.log(`Associated professor ${user.id} with faculty ${faculty.id}`);
            }
        }
    } catch (error) {
        console.error(`Error associating professor ${user.id} with faculty:`, error);
    }
}
