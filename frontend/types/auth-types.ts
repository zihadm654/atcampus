...
export function hasRole(user: ExtendedUser, role: CustomUserProperties["role"]): boolean {
    return user.role === role;
}

// Type guard to check if user has one of several roles
export function hasAnyRole(user: ExtendedUser, roles: CustomUserProperties["role"][]): boolean {
    return roles.includes(user.role);
}

// Type guard to check if user has a specific status
export function hasStatus(user: ExtendedUser, status: CustomUserProperties["status"]): boolean {
