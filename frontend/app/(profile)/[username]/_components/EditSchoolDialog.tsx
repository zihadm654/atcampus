...
export default function EditSchoolDialog({
  ...
  const form = useForm<TSchool>({
    ...
    defaultValues: school
      ? {
          ...
          slug:
            school.slug ||
            school.name.trim().toLowerCase().replace(/\s+/g, "-"),
  ...
  const createFacultyMutation = useUpdateFacultyMutation();
  ...
  async function onSubmit(values: TSchool) {
    try {
      ...
      // Handle faculties using mutations
      ...
      const removedFaculties =
        school?.faculties?.filter(
          (f) => !values.faculties.some((v) => v.id === f.id),
        ) || [];
      for (const f of removedFaculties) {
        await deleteFacultyMutation.mutateAsync(f.id);
      }
      onOpenChange(false);
    } catch (error) {
      console.error(error);
