...
import { useAcceptFollowRequest, useRejectFollowRequest } from "@/hooks/useFollowRequests";
import { useUser } from "@/hooks/useUser";
import { FollowRequestCard, EmptyFollowRequests } from "@/components/follow/FollowRequestCard";
...
export default function Connections() {
  ...
  const { user } = useUser();
  const [activeTab, setActiveTab] = useState<"following" | "followers" | "mutual" | "requests">("following");

  // Fetch following users
  const { data: followingData, isLoading: followingLoading } = useQuery({
    queryKey: ["user-following"],
    queryFn: async () => {
      const response = await kyInstance.get("/api/users/following").json<{ users: User[] }>();
  ...
  const { data: followersData, isLoading: followersLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await kyInstance.get("/api/users/followers").json<{ users: User[] }>();
      return response.users;
    },
  });
  ...
  const { data: suggestionsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await kyInstance.get("/api/users/suggestions").json<{ suggestions: User[] }>();
  ...
  const { data: followRequestsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await kyInstance.get(`/api/users/${user?.id}/follow-requests`).json<{ followRequests: any[] }>();
  ...
  const mutualConnections = followingData?.filter((followedUser) =>
    followersData?.some((follower) => follower.id === followedUser.id)
  ) || [];

  const isLoading = followingLoading || followersLoading;
  const pendingRequests = (followRequestsData?.followRequests || []).filter(req => req.status === "PENDING" && req.targetId === user?.id);
    ...
    <div className="space-y-4">
      {requests.map((request) => (
        <FollowRequestCard
          key={request.id}
          request={request}
          onAccept={(requesterId) => acceptFollowRequest.mutate({ userId: requesterId, targetUserId: request.targetId })}
          onReject={(requesterId) => rejectFollowRequest.mutate({ userId: requesterId, targetUserId: request.targetId })}
          isLoading={acceptFollowRequest.isPending || rejectFollowRequest.isPending}
        />
      ))}
    </div>
  );
