...
export default function FollowRequestsPage() {
  ...
  const { data: followRequestsData, isLoading } = useFollowRequests(user?.id || "");
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <p className="text-muted-foreground">Please log in to view follow requests.</p>
      </div>
    );
  }
  ...
  const { received: receivedRequests, sent: sentRequests } = filterFollowRequests(allRequests, user.id);
